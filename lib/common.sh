...
echo "ERROR: Source file $1 does not exist. Check fetch_from_github configuration."
exit
fi
sync
}
...
debootstrap --include=openssh-server,debconf-utils --arch=armhf --foreign $RELEASE $DEST/output/sdcard/ http://ftp.us.debian.org/debian
...
# install aditional packages
	PAKETKI="alsa-utils automake bash-completion bc bridge-utils bluez build-essential cmake cpufrequtils curl dosfstools evtest figlet fping git haveged hddtemp hdparm hostapd htop i2c-tools iperf ir-keytable iw less libbluetooth-dev libbluetooth3 libtool libwrap0-dev libfuse2 libnl-dev libssl-dev lirc lsof makedev module-init-tools mtp-tools nano ntp parted pkg-config pciutils pv python-smbus rfkill rsync screen stress sudo sysfsutils u-boot-tools unzip usbutils wireless-tools wget wpasupplicant"
